...
| 4 | Le nouveau serveur **remplace** le serveur Node/Mongo de Hots-Overlay (décommission en fin de V1) ; extension Twitch et overlay re-pointés. |
...
- Tests : corpus de replays de référence multi-builds (2023→2026) commité en Git LFS ou téléchargé.
...
- **Dump intégral** : `GET /api/matches/{id}/raw?stream=…` — décode le fichier archivé à la volée,
  cache disque compressé **LRU borné** (ex. 5 GB) ; jamais de pré-décodage massif.
- **Overlay/Twitch** : reprend les routes nécessaires à l'extension Twitch + le panneau
  patch-digest (poussé par HotsPatchNotes — conservé tel quel).
- **Jarvis** : publie `hots.match.completed` (payload : résultat, héros, KDA, carte, durée,
  awards, écarts vs moyennes) sur le Redis du box.
...
3. **Serveur + DB + backfill** — upload/fingerprint/workers/Postgres/WS/raw+LRU/admin/tokens ;
   mode backfill client-rs. *Accept : archive complète importée ; fin de partie → page < 5 s.*
...
5. **Stream + Jarvis + bascule** — widget OBS, `hots.match.completed` → Redis, connecteur Jarvis,
   extension Twitch re-pointée, **décommission Node/Mongo**. *Accept : partie jouée → widget OBS
   + brief Jarvis ; ancien serveur arrêté sans perte de fonction.*
