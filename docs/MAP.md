...
| `ingest/` | Docker events → stream (M2); metrics poller → `metrics` + signal events (P2); topology builder → `topology` edges (P2) | `docker_events.py`, `metrics.py`, `topology.py` | M2, P2 |
